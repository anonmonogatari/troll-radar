...
async function loadNarrativeBriefing() {
    ...
    try {
        const res = await fetch(`/api/narratives?days=${currentDays}`);
        const data = await res.json();
...
async function loadAnalytics() {
    try {
        ...
        // 3. Heatmap
        const heatRes = await fetch(`/api/heatmap?days=${currentDays}`);
        const heatData = await heatRes.json();
        renderHeatmap(heatData.heatmap || []);

        // 4. Keywords
        const keyRes = await fetch(`/api/keywords?days=${currentDays}&limit=30`);
...
async function loadNetwork() {
    ...
    try {
        const res = await fetch(`/api/coordination?days=${currentDays}`);
        const data = await res.json();
...
async function loadAuthors() {
    try {
        const res = await fetch(`/api/authors?days=${currentDays}`);
        const data = await res.json();
